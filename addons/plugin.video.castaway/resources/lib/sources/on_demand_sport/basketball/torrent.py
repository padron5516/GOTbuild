...
class info():
    def __init__(self):
    	self.mode = 'torrent'
        self.name = 'Basketball torrents (full replays)'
...
class main():
	def __init__(self,url = 'http://www.sport-video.org.ua/basketball.html'):
	...
	def next_page(self):
		try:
			page = re.findall('basketball(\d+).html',self.url)[0]
			next_page = self.url.replace('basketball%s.html'%(page),'basketball%s.html'%(int(page)+1))
		except:
			next_page = self.url.replace('basketball.html','basketball1.html')
		return next_page
