...
class main():
	def __init__(self,url = 'http://www.sport-video.org.ua/soccer.html'):
	...
	def next_page(self):
		try:
			page = re.findall('soccer(\d+).html',self.url)[0]
			next_page = self.url.replace('soccer%s.html'%(page),'soccer%s.html'%(int(page)+1))
		except:
			next_page = self.url.replace('soccer.html','soccer1.html')
		return next_page
