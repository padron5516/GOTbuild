...
class info():
    def __init__(self):
    	self.mode = 'torrent'
        self.name = 'Hockey torrents (full replays)'
...
class main():
	def __init__(self,url = 'http://www.sport-video.org.ua/hockey.html'):
	...
	def next_page(self):
		try:
			page = re.findall('hockey(\d+).html',self.url)[0]
			next_page = self.url.replace('hockey%s.html'%(page),'hockey%s.html'%(int(page)+1))
		except:
			next_page = self.url.replace('hockey.html','hockey1.html')
		return next_page
