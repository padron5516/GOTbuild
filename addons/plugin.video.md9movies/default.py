...
def CAT():
        ...
        addDir('[B][COLOR white]Release Date[/COLOR][/B]',baseurl+'/filter?sort=release&type%5B%5D=movie',1,icon,fanart,'')
...
def EPIS(url):
        ...
        for url,data_id,name in match:
                ...
                if ok in url:
                        ...
                        headers = {'host':'9movies.to', 'referer': url, 'user-agent':User_Agent, 'x-requested-with':'XMLHttpRequest'}
                        ...
                        url = link.replace('\/','%2F').replace(':','%3A').replace('=','%3D').replace('@','%40')
...
def LINK(url):
        link = OPEN_URL(url)
        link = link.encode('ascii', 'ignore')
        match = re.findall(r'<li><a.*?href="(.*?)" data-id="(.*?)".*?>(.*?)</a>', str(link), re.I|re.DOTALL)[1:]
        for url,data_id,name in match: 
                ok = 'film/'
                if ok in url:
                        url = baseurl+url
                        ...
                        headers = {'host': '9movies.to', 'content-type':'application/json, text/javascript, */*; q=0.01',
                                   'referer': url, 'user-agent':User_Agent, 'x-requested-with':'XMLHttpRequest'}
                        r = requests.get('http://9movies.to/ajax/film/episode?hash_id='+data_id, data=form_data, headers=headers).text
                        link = re.compile('"videoUrlHash":"(.*?)"').findall(r)[0]
                        url = link.replace('\/','%2F').replace(':','%3A').replace('=','%3D').replace('@','%40')
                        addDir('[B][COLOR white]%s[/COLOR][/B]' %name,url,11,icon,fanart,'')
...
def LINK2(url):
        headers = {'host':'player.apptwist.net', 'origin':'http://9movies.to',
                   'referer':url, 'user-agent':User_Agent}
        r = requests.get('http://player.apptwist.net/grabber/?flash=1&link='+url+'&json=1', headers=headers).text
        try:
                try:
                        url = re.findall(r'"label":".*?","file":"(.*?)"', str(r), re.I|re.DOTALL)[-1]
                except:
                        url = re.findall(r'"label":".*?","file":"(.*?)"', str(r), re.I|re.DOTALL)[0]
        except:pass
