...
CONTEXT_PRIORITIES = enum(WATCHFLAG=1, FAVORITE=30, WATCHLIST=30, FAVLIST=31, CUSTOMLIST=32, SIMILAR=11, SEASONS=12, TRAILER=10, CACHEFILE=41, PLAY_CACHE=42, DELETE_CACHE=43, SCRAPERS=90, FILTERS=91, VISIBILITY=92, SET_VIEW=95, WALTER=99, TRANSMOGRIFY=100, TM_STREAMING=105, TM_QUEUE=106, SETTINGS=1000)
...
def set_source_filter():
	...
	items = [QUALITY.r_map[i] for i in range(1,10)] + ["AllDebrid", "RealDebrid", "Premiumize.ME", "RPNET"]
...
def on_playback_stop():
	...
	#navigate_back = False if ( ADDON.get_setting('source_selection_mode') == 'Dialog' or plugin.is_playlist() is False) else True 
	#refresh = False if ( ADDON.get_setting('source_selection_mode') == 'Directory' or plugin.is_playlist() ) else True

	refresh = False
	navigate_back = False

	if ADDON.get_setting('enable_resume') == 'true':
			hash_id = plugin.get_hash_id(plugin.get_arg('imdb_id',''),plugin.get_arg('tmdb_id',''),plugin.get_arg('season', ''),plugin.get_arg('episode', ''))
...
def get_episode_sources():
	...
	for source in sources:
		...
		menu.add('Transmogrify', {'mode': 'tv_directory_transmogrify', "url": raw_url, "display": title, "imdb_id": imdb_id,"tmdb_id": tmdb_id, "year": year, "showtitle": title, "season": season, "episode": episode}, visible=validate_transmogrifier, priority=100)	
...
def get_movie_sources():
	...
	for source in sources:
		...
		if quality < 6:
			...
			image = vfs.join(ARTWORK, 'definition/720.png')
		menu.add('Transmogrify', {'mode': 'movie_directory_transmogrify', "url": raw_url, "display": title, "imdb_id": imdb_id, "tmdb_id": tmdb_id, "year": year, "title": title}, visible=validate_transmogrifier, priority=100)	
