...
def YOUTUBE_LIST():
        ...
        addDirPlay("Justin Sandercoe",'plugin://plugin.video.youtube/user/%s/' % channel,13,art1 + 'youtubejs.png')#+'/uploads?start-index=1&max-results=20'
        channel="martyzsongs"
        addDirPlay("Marty Schwartz",'plugin://plugin.video.youtube/user/%s/' % channel,13,art1 + 'youtubems.png')
        channel="rockongoodpeople"
        addDirPlay("Next Level Guitar",'plugin://plugin.video.youtube/user/%s/' % channel,13,art1 + 'youtubenlg.png')
        channel="youcanlearnguitar"
        addDirPlay("You Can Learn Guitar",'plugin://plugin.video.youtube/user/%s/' % channel,13,art1 + 'youtubeyclg.png')
        channel="guitarings"
        addDirPlay("Guitarings",'plugin://plugin.video.youtube/user/%s/' % channel,13,art1 + 'youtubegr.png')
        channel="GaragebandandBeyond"
        addDirPlay("GarageBand & Beyond",'plugin://plugin.video.youtube/user/%s/' % channel,13,art1 + 'youtubegb.png')
        channel="wickedwkd"
        addDirPlay("Pearl Jam covers by wickedwkd",'plugin://plugin.video.youtube/user/%s/' % channel,13,art1 + 'pjcovers.png')
...
def PLAY_VIDEO(url,name):
        ...
        try:
            ...
            vurl = match[0].replace('http://www.youtube.com/watch?v=', '')
        except:
            match = re.compile('<iframe width="600" height="400" src="https://www.youtube.com/embed/(.+?)" frameborder="0" allowfullscreen></iframe>').findall(link)
            vurl = match[0]
...
def addDirPlay(name,url,mode,iconimage):
        u=sys.argv[0]+"?url="+urllib.quote_plus(url)+"&mode="+str(mode)+"&name="+urllib.quote_plus(name)
        ok=True
        liz=xbmcgui.ListItem(name, iconImage="DefaultFolder.png", thumbnailImage=iconimage)
        liz.setInfo( type="Audio", infoLabels={ "Title": name } )
